...
from traceloop.sdk.decorators import task, workflow, tool
...
class ChatBot:
    ...
    @task(name="execute")
    def execute(self):
        # Build the full prompt including system message and conversation history
        full_prompt = self.system + "\\n\\n"
        for msg in self.messages:
            if msg["role"] == "user":
                full_prompt += f"Question: {msg['content']}\\n"
            else:
...
@tool(name="chat_bot_google")
def google(query):
    headers = {
        "X-API-KEY": os.environ["SERPER_API_KEY"],
        "Content-Type": "application/json",
    }
    payload = {"q": query, "num": 5}  # Number of search results to return
...
def query(question, max_turns=3):
    ...
    while i < max_turns:
        ...
        if actions:
            ...
            if action not in get_known_actions():
                continue
                # raise Exception("Unknown action: {}: {}".format(action, action_input))
            logger.info(" -- running {} {}".format(action, action_input))
            observation = get_known_actions()[action](action_input)
...
if __name__ == "__main__":
    try:
        ...
        logger.error(f"Traceloop exception: {error}")
        print(f"Error: {error}")
